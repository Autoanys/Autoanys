...
const Flowbar = ({ sidebarOpen, setSidebarOpen }: SidebarProps) => {
  ...
  return (
    <aside
      ref={sidebar}
      className={`w-58 absolute right-0 top-0 z-9999 flex h-screen flex-col overflow-y-hidden bg-slate-50 duration-300 ease-linear dark:bg-zinc-900	 lg:static lg:translate-x-0 ${
        sidebarOpen ? "translate-x-0" : "-translate-x-full"
      }`}
    >
      {/* <!-- SIDEBAR HEADER --> */}
      <div className="flex items-center justify-between gap-2 bg-slate-50 px-6 py-4 pb-4.5 shadow-1 drop-shadow-1 dark:bg-zinc-900 dark:drop-shadow  ">
        <Link href="/">
          <Image
      ...
      <div className="no-scrollbar flex flex-col overflow-y-auto duration-300 ease-linear">
        ...
        <nav className="mt-5 px-4 py-4 lg:mt-9 lg:px-6">
          {/* <!-- Menu Group --> */}
          <div>
            <h3 className="mb-4 ml-4 text-sm font-semibold text-bodydark2 dark:text-blue-600">
              Welcome to AutoAnys
            </h3>
                  ...
                  return (
                    <React.Fragment>
                      <Link
                        ...
                        font-medium font-semibold text-black duration-300 ease-in-out hover:bg-white hover:shadow-2xl dark:hover:bg-meta-4 ${
                          (pathname === "/auth" || pathname.includes("auth")) &&
                          "shadown-2xl bg-white dark:bg-meta-4"
                        }`}
                        onClick={(e) => {
                          e.preventDefault();
                          sidebarExpanded
                      ...
                      >
                        ...
                        Authentication
                        <svg
                          className={`right- absolute top-1/2 -translate-y-1/2 fill-current ${
                            open && "rotate-180"
                          }`}
                          width="20"
                          height="20"
