...
const Sidebar = ({ sidebarOpen, setSidebarOpen }: SidebarProps) => {
  ...
  return (
    ...
    <aside
      ref={sidebar}
      className={`w-58 absolute left-0 top-0 z-9999 flex h-screen flex-col overflow-y-hidden bg-slate-50 duration-300 ease-linear dark:bg-boxdark lg:static lg:translate-x-0 ${
        sidebarOpen ? "translate-x-0" : "-translate-x-full"
      }`}
    >
      {/* <!-- SIDEBAR HEADER --> */}
      <div className="px-6.1 flex items-center justify-between gap-2 bg-slate-50 py-4.5 shadow-1 drop-shadow-1  ">
        <Link href="/">
          <Image
                    ...
                    <React.Fragment>
                      <Link
                        ...
                        font-medium font-semibold text-black duration-300 ease-in-out hover:bg-white hover:shadow-2xl dark:hover:bg-meta-4 ${
                          ...
                          "shadow-2xls bg-white dark:bg-meta-4"
                        }`}
                        onClick={(e) => {
                          e.preventDefault();
                          sidebarExpanded
